...
{
    if (items.empty()) std::cout << "인벤토리가 비어있음" << std::endl;
    else
    {
        ...
        {
            std::cout << i + 1 << ". " << items[i].GetName() << " - " << items[i].GetPrice() << "골드" << std::endl;
        }
...
Item Inventory::SellItem(const int index, int& gold)
{
    ...
    std::cout << soldItem.GetName() << "을 판매. 금액: " << price << std::endl;
...
void Inventory::EquipItem(const int index)
{
    ...
    {
        equipWeapon = item;
        std::cout << item.GetName() << " 무기를 장착!" << std::endl;
    }
    else if (item.GetType() == GameEnums::ItemType::ITEM_ARMOR)
    {
        equipArmor = item;
        std::cout << item.GetName() << " 방어구를 장착!" << std::endl;
    }
